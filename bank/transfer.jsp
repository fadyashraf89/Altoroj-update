    ...
    <td valign="top" colspan="3" class="bb">
		...
		<table cellSpacing="0" cellPadding="1" width="100%" border="0">
		    ...
		    <td>
			  <select size="1" id="fromAccount" name="fromAccount">
				  <c:forEach var="account" items="${user.getAccounts()}">
					  <option value="${fn:escapeXml(account.getAccountId())}">
							  ${fn:escapeXml(account.getAccountId())} ${fn:escapeXml(account.getAccountName())}
					  </option>
				  </c:forEach>
		    ...
		    <td>
			  <select size="1" id="toAccount" name="toAccount">
				  <c:forEach items="${user.accounts}" var="account">
					  ...
					  <c:set var="accountName" value="${account.accountName}" />
					  <option value="<c:out value="${accountId}" />">
						  <c:out value="${accountId}" /> <c:out value="${accountName}" />
					  </option>
				  </c:forEach>
			  </select>
		    </td>
		  ...
		  <tr>
		    ...
		    <span id="soapResp" name="soapResp" align="center"></span>
		    </td>
		  </tr>
		</table>
		</form>
