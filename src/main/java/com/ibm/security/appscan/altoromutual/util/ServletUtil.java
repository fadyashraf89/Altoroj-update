...
public class ServletUtil {
    ...
    public static String[] searchArticles(String query, String path) {
        ...
        if (query == null || path == null || query.isEmpty() || path.isEmpty()) {
            return null;
        }
        ...
        try {
            // Load XML document from the specified path
            ...
            if (nodes.getLength() == 1) {
                ...
                // Iterate over child nodes to search for matching articles
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    if ("publication".equals(node.getNodeName())) {
                        ...
                        // Extract title and isPublic values
                        for (int x = 0; x < innerNodes.getLength(); x++) {
                            Node innerNode = innerNodes.item(x);
                            String nodeName = innerNode.getNodeName();

                            if ("title".equals(nodeName)) {
                                title = innerNode.getTextContent();
                            } else if ("isPublic".equals(nodeName)) {
                                isPublic = Boolean.parseBoolean(innerNode.getTextContent());
                            }

                            if (title != null && isPublic != null) {
                                // Check if the article is public and contains the query
    ...
    private static ArrayList<File> searchFiles(String query, File rootFile) {
        // error checking
        if (!rootFile.canRead()) return new ArrayList<File>();
    ...
    public static boolean isAppPropertyTrue(String property) {
        ...
        return tokenizer.countTokens() == 1 && tokenizer.nextToken().equals("true");
    }
    ...
    public static void initializeAppProperties(ServletContext servletContext) {
        ...
        if (propertyFileStream == null) return;
    ...
    public static Cookie establishSession(String username, HttpSession session) {
        try {
            ...
            // Use parameterized query to retrieve user information
            ...
            // Convert accounts to a base64 string
            String accountStringList = Account.toBase64List(accounts);

            // Create a cookie with the account information
    ...
    static public void initializeRestAPI(ServletContext servletContext) {
        ...
        try {
            ...
            //no update needed
            if (newBasePath.equals(swaggerProps.getString(basePath))) return;
